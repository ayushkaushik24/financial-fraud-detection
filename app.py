...
st.sidebar.info("● Cloud Sentinel: Deployed")

st.sidebar.markdown("---")
st.sidebar.markdown("#### 📊 System Metrics")
st.sidebar.metric("Monitored Volume", "284,807")
st.sidebar.metric("Anomalies Mitigated", "492")
...
with tab1:
    ...
    if analyze_btn:
        with st.spinner("Analyzing neural pathways & feature vectors..."):
            if "High Risk" in sample_type or tx_amount > 10000:
                risk_score = np.random.uniform(88.5, 99.2)
            else:
                risk_score = np.random.uniform(0.1, 8.4)
...
with tab2:
    ...
    # Mock data for cloud visualization
    mock_df = pd.DataFrame({
        'Time': [12, 45, 88, 120, 150, 200, 230, 290, 310, 350],
        'Amount': [150.0, 2500.0, 89.0, 12000.0, 45.0, 99.0, 8500.0, 320.0, 12.0, 6400.0],
        'Class': [0, 0, 0, 1, 0, 0, 1, 0, 0, 1]
    })

    with col_a:
        st.markdown("#### Sample Processed Transactions")
        st.dataframe(mock_df, use_container_width=True)
    
    with col_b:
        st.markdown("#### Average Spending: Normal vs Fraud (₹)")
        chart_data = pd.DataFrame({
            'Category': ['Normal (0)', 'Fraud (1)'],
            'Avg Amount': [88.34, 122.21]
        }).set_index('Category')
